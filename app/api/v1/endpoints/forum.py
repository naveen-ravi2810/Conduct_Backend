...
from app.models.forum import ForumCreate, Forum, ReadForum
...
async def get_every_comments(
    ...
    forums = await get_every_comments_without_id(
        session=session, user_id=token_details.id
    )
    return paginate(forums)
