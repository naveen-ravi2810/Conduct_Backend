...
from app.modules.models import UserCreate, UserURIUpdate, LoginUser, ShowUserProfile
...
from app.curd.curd_user import (
    create_user,
...
@router.delete("/logout", status_code=status.HTTP_200_OK, response_model=ResponseSchema)
...
@router.post(
    "/register", status_code=status.HTTP_201_CREATED, response_model=ResponseSchema
)
...
# ?????????????????????????????????????????????????????????????????????????
@router.patch(
    "/profile_picture", status_code=status.HTTP_200_OK, response_model=ResponseSchema
)
def update_profile_picture(
    ...
    return ResponseSchema(message=file.filename)
