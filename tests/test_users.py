...
def test_current_users(client:TestClient, user_token_headers):
    r = client.get(f'{settings.PROJECT_ENDPOINT_VERSION}/profile/1',headers=user_token_headers)
...
def test_other_user(client:TestClient, user_token_headers):
    r = client.get(f'{settings.PROJECT_ENDPOINT_VERSION}/profile/0',headers=user_token_headers)
